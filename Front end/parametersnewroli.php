...
//mysql_select_db($con,"admindata")or die("db not found");
	...
	$PCity=$_POST['patient_city'];
	$Pcountry=$_POST['country'];
	$Pstate=$_POST['state'];
	$Pzip=$_POST['Patient_postal'];
	...
	$Estate=$_POST['state3'];
	$Ezip=$_POST['APatient_postal'];
	
	$_SESSION['patient_name']=$Pname;
	$_SESSION['dob']=$bod;
	
	//echo "here";
	$sql  = "INSERT INTO `patient` (`Patient_id`, `Name`, `Fathers_name`, `Gender`, `Dob`, `Marital_status`, `Mobile`, `Email`, `PStreet`, `PCity`, `PZip`, `PState`, `PCountry`, `TStreet`, `TCity`, `TZip`, `TState`, `TCountry`, `EStreet`, `ECity`, `EZip`, `EState`, `ECountry`, `EMobile`, `EEmail`) VALUES (NULL, '$Pname', '$PFname', '$pgender1', '$bod', '$status1', '$mobile', '$email', '$PStreet', '$PCity', '$Pzip', '$Pstate', '$Pcountry', '$TStreet', '$TCity', '$Tzip', '$Tstate', '$Tcountry', '$EStreet', '$ECity', '$Ezip', '$Estate', '$Ecountry', '$Ephone', '$Eemail')";
...
<script type="text/javascript">
			...
			function Checkneckc(){
				if(document.getElementById('neckc').value==''||document.getElementById('neckc').value>30){
					  document.getElementById('errorneckc').innerHTML="enter value between 1 and 30";
...
<form action="http://localhost:12345/datafinal"  name="myForm" onsubmit="return validateForm()" method="post">	
<!--<div class="tab">
...
<table align="center" bgcolor="#EEF1FB"  width="1600" height="100" box-shadow="20px 20px"  id="rcorners2">
<tr>
<td>
...
<td> <input height="100px" type="date" name="dateofa" id="dateofa"> </td>
 <td><label name="ward"> Ward No </label></td>
 <td> <input height="100px" type="text" name="wardno" id="wardid"> </td>
   <td></td>
   <td></td>
   <td></td>
...
<lable>Neck Circumference</lable>
 ...
 <td>
   <input type="float" id="neckc" name="nc" placeholder="Neck Circumference" onblur="Checkneckc();" pattern ="^[0-9]*\.?[0-9]*+$"  min="1" max="30" style="width:150px" required>
    </td>
...
<div id="Intubation" class="tabcontent">
  ...
  <td nowrap ><lable>Thyromental</lable></td>
  <td> 
  ...
  <td> <select class="form-dropdown" name="neck_thick" style="width:150px" required>
    ...
    <td nowrap ><lable>Absense of mouth opening</lable></td>
