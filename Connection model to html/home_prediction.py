...
@app.route("/home_prediction", methods=["GET", "POST"])
def get_data():
    if lr and lv:
        try:
                 ...
                 if nr==1:
                     ...
                     nra="  Absent"
                 return render_template('home_predictionresult.php',placeholder="pre",a=str(res),b=str(res1),c=str(str1),age=str(age),sex=str(gen),bmi=bmii,ms=str(mal),ig=str(ig),td=str(td),hnd=str(Hnm),p=str(peg),bt=str(den),nt=str(neckthick),mo=str(ao),sh=str(shi),fa=str(fas),sm=str(sma),ub=str(ulbt),be=str(beard),nrl=nra,t=str(teeth),nc=str(nc),sl=str(stiff_lungs),hs=str(hos),mg=str(macroglossia))
        except:
