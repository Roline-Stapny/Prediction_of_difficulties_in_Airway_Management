    ...
    if lr and lv:
        try:
            ...
            # print("session date of birth is ", session.get('dob'))
                 ...
                 mycursor.execute(
                "INSERT INTO `patient_parameters` (`Patient_id`, `Date_of_admission`, `Age`, `Gender`, `Beard`, `Mallampati`, `ULBT`, `BMI`, `Neck_circumference`, `Neck_radiation`, `Stiff_lungs`, `History_of_snoring`, `Teeth`, `Macroglossia`, `Interincisor_gap`,`Head_and_neck_movement`, `Pregnant`, `Neck_thick`, `Face_asymmetry`,`Small_mandible`, `Thyromental_distance`, `strider_history`, `buck_teeth`,`absence_of_mouth_op`) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                (Patient_id, doa, age, gender, beard, mal, ulbt, bmi, nc, nr, stiff_lungs, hos, teeth, macroglossia, ig, Hnm, p, nt, fa, sm, td, sh, dc, afo))
                 mydb.commit()
